...
export const ListUsersAwaitingApproval: React.FC = () => {
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [usersAwaitingApprovals, setUsersAwaitingApprovals] = useState<
    User[] | []
  >([]);
  const [, setUsersAwaitingApproval] = useLocalStorage<User[] | []>(
    [],
    'usersAwaitingApproval',
  );

  const onCarouselClick = (userId: number) => {
    window.location.href = `/pages/admin/registration-requests/${userId}?action=usersAwaitingApproval`;
  };
