...
export async function POST(
    ...
    try {
        ...
        const valueTopic = { ...request, status: status, userId: parseInt(userId) as number };
...
export async function GET(
    ...
    try {
        const { status, userId } = params;
        const topics = await handleGetAllTopics({ status: status, userId: parseInt(userId) as number });
