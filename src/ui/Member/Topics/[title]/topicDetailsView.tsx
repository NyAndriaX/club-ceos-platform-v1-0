...
export const TopicDetailsView = () => {
    ...
    const [isFetchingTopicReply, setIsFetchingTopicReply] = useState<boolean>(false);
    const [topicReplies, setTopicReplies] = useState<(ReplyOutput & { author: UserOutput })[] | []>([]);
    ...
    const backgroundImage = useMemo(() => {
        return topicData?.topic?.coverImage || topicData?.topicType?.coverImage || '';
    }, [topicData]);
    ...
    useEffect(() => {
        const fetchTopicDetails = async () => {
            ...
            try {
                ...
                if (!topicRes.ok) throw new Error('Erreur lors de la récupération du sujet');
                const { topic: fetchedTopic } = await topicRes.json() as { topic: TopicOutput };
                ...
                if (!topicTypeRes.ok) throw new Error('Erreur lors de la récupération du type de sujet');
                if (!themeRes.ok) throw new Error('Erreur lors de la récupération du thème');
                ...
                setTopicData({ topic: fetchedTopic, topicType: fetchedTopicType, theme: fetchedTheme, user: fetchedUser });
            } catch (error) {
    ...
    useEffect(() => {
        const fetchTopicReply = async () => {
            ...
            try {
                ...
                if (!topicReplyRes.ok) throw new Error("Erreur lors de la récupération des réponses de ce sujet");

                const { topicReplies: fetchedTopicReplies } = await topicReplyRes.json();
                setTopicReplies(fetchedTopicReplies);
            } catch (error) {
                console.error("Erreur lors de la récupération des réponses de ce sujet :", error);
            } finally {
    ...
    const onReply = async (values: ReplyInput) => {
        ...
        try {
            const response = await fetch(`/api/topic/reply/${topic?.id}/${user?.id}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(values)
            })
            if (!response.ok) throw new Error("Erreur lors de l'enregistrement de réponse");
                ...
                <MemberPage
                    header={
                        <div className="flex justify-between items-center w-full">
                            <h1 className="text-2xl text-gray-900 font-bold max-w-[70%] line-clamp-3">{topic.title}</h1>
                            <Link href="/member/home" className="flex flex-row gap-2 items-center border-b border-blue-500 text-sm font-normal text-blue-500">
                                <span className="pi pi-arrow-left" style={{ fontSize: '0.75rem' }} />
                                <span>Revenir à l&apos;accueil</span>
                            </Link>
                        </div>
                    }
                    className="max-w-6xl bg-white z-40 shadow-sm" headerClassName="bg-white"
                >
                    <div className="flex flex-row items-start p-4 justify-between gap-16">
                        <div className="flex flex-col gap-8 items-start w-full">
                            <div dangerouslySetInnerHTML={{ __html: topic.content }} className="border-b border-gray-200 text-gray-500 pb-12 w-full" />

                            <div className="flex flex-col gap-4 items-start w-full">
                                <h3 className="text-lg font-semibold text-gray-900">{
                                    !isFetchingTopicReply ? topicReplies.length : <ProgressSpinner style={{ width: "20px", height: "20px" }} strokeWidth="4" />
                                } réponse</h3>
                                <div className="flex flex-col gap-4 w-full">
                                    {
                                        isFetchingTopicReply ? (
                                            ...
                                            {
                                                topicReplies.slice(0, visibleCount).map((topicReply, index: number) => (
                                                    <div key={index} className="flex flex-col gap-6 border-b border-gray-100 w-full pb-4">
                                                        <div className="flex flex-row items-center gap-4">
                                                            ...
                                                            />
                                                            <span className="text-lg font-bold text-gray-900">{topicReply.author.firstName} {topicReply.author.lastName}</span>
                                                        </div>
                                                        <div dangerouslySetInnerHTML={{ __html: topicReply.content }} className="text-base font-normal text-gray-700 pl-8" />
                                                    </div>
                                                ))
                                            }
                                ...
                                >
                                    {({ errors, touched, setFieldValue }) => (
                                        <Form className="flex flex-col gap-8 items-start w-full">
                                            ...
                                            <Button type='submit' label='Répondre' disabled={isLoading} loading={isLoading} />
                                        </Form>
                                    )}
                                </Formik>
                            </div>
                        ...
                        <div className="flex flex-col gap-6 min-w-[300px]">
                            <div className="flex flex-row gap-2 items-center justify-between w-full">
                                {dominantColor && <div className="h-3 w-3 rounded-full" style={{ backgroundColor: dominantColor }} />}
                                <span className="text-lg font-semibold text-gray-900 line-clamp-2 w-[90%]">{theme?.title}</span>
                            </div>
                            <div className="flex flex-row gap-4 p-2 items-start">
                                ...
                                <div className="flex flex-col gap-4 items-start">
                                    <div className="flex flex-col">
                                        <p className="text-base text-gray-900 font-semibold">{user?.firstName} {user?.lastName}</p>
                                    ...
                                    <div className="flex flex-row gap-4 items-center">

                                        {
                                            user && user.linkedInUrl && (
                                                <Link href={user?.linkedInUrl as string} target="_blank" rel="noopener noreferrer">
                                                    <span className="pi pi-linkedin text-blue-900" style={{ fontSize: '1.5rem' }}></span>
                                                </Link>
                                            )
                                        }
                                        {
                                            user && user.companyWebsite && (
                                                <Link href={user.companyWebsite} target="_blank" rel="noopener noreferrer">
                                                    <span className="pi pi-globe text-blue-900" style={{ fontSize: '1.5rem' }}></span>
                                                </Link>
                                            )
                                        }
                                    </div>
                                    <div className="flex flex-col gap-2 items-start text-sm text-blue-500">
                                        <Link href={'/'} className="flex flex-row items-start gap-4">
                                            <span className="underline">Supprimer définitivement</span>
                                            <span className="pi pi-trash"></span>
                                        </Link>
                                        <Link href={'/'} className="flex flex-row items-start gap-4">
                                            <span className="underline">Editer un sujet
                                            </span>
