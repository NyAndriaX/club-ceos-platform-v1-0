...
export const ProfilView = () => {
    ...
    const [topics, setTopics] = useState<{ topic: TopicOutput; topicType: TopicTypeOutput, theme: ThemeOutput }[]>([]);
            ...
            try {
                ...
                const topicsWithType = await Promise.all(
                    fetchedTopics.map(async (topic: TopicOutput) => {
                        const responseAsFetchTopicType = await fetch(`/api/topic/type/${topic.topicTypeId}`);
                        const { topicType: fetchedTopicType } = await responseAsFetchTopicType.json();
                        const responseAsFetchTheme = await fetch(`/api/theme/${topic.themeId}`);
    ...
    const defaultProfileImage = "https://primefaces.org/cdn/primereact/images/avatar/amyelsner.png";
                    ...
                    <div className="flex justify-between items-center w-full">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-gray-500 font-light text-base">{user.email}</p>
                            </div>
                        </div>
                        <div className="text-center">
                            <h2 className="text-lg font-semibold">{dayjs(user.createdAt).format("DD/MM/YYYY")}</h2>
                    ...
                    <div className="flex flex-col gap-4 p-4">
                        ...
                        <div className="flex flex-col gap-4 p-2">
                            {isFetchingTopics ? (
                                ...
                                <p className="text-gray-500 text-center">Aucune activité n&apos;est disponible.</p>
                            ) : (
                                topics.map(async ({ topic, topicType, theme }, index) => {
                                    return (
                                        <div
                                            ...
                                            onClick={() =>
                                                router.push(`/member/topics/${encodeURIComponent(topic.title)}?topicId=${encodeURIComponent(topic.id)}`)
                                            }

                                        >
                                            <Image
                                                src={topic.coverImage ?? (topicType.coverImage as string)}
                                                ...
                                                <div className="flex flex-col gap-2 h-full w-full">
                                                    ...
                                                    <div className="flex flex-row items-end w-full">
                                                        <div className="flex-1">
                                                            <span className="text-base text-gray-700 line-clamp-2" dangerouslySetInnerHTML={{ __html: topic.content.replace(/<\/?p>/g, "") }} />
                                                            <Link href={`/topic/${topic.id}`} className="underline text-xs text-blue-500 ml-1">
                                                                Lire la suite
                                                            </Link>
                                                        </div>
                                                    </div>
                                                ...
                                                <div className="flex flex-row w-full justify-between items-center">
                                                    <div className='flex flex-row items-center gap-4 text-xs text-gray-500'>
                                                        <div className="flex flex-row gap-2 items-center">
                                                            <div className="h-3 w-3 rounded-full" style={{ backgroundColor: dominantColors[index] }} />
                                                            <span className="line-clamp-2 w-48">{theme.title}</span>
                                                        </div>
                                                        <Tag value={topicType.name} style={{ backgroundColor: dominantColors[index] }} />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
