...
export const NewTopicTypeForm: React.FC = () => {
    ...
    const uploadFile = async (file: File): Promise<string | null> => {
        try {
            ...
            showToast('error', 'Erreur de téléversement', 'Impossible de téléverser le fichier. Veuillez réessayer.');
            return null;
        }
    };

    const handleSubmit = async (values: TopicTypeInput, resetForm: () => void) => {
        setIsLoading(true);
        try {
            ...
            if (!response.ok) throw new Error("Une erreur est survenue lors de l'enregistrement.");

            showToast('success', 'Succès', 'Le type de thématique a été enregistré avec succès.');
            resetForm();
        } catch (error) {
            showToast('error', 'Erreur', "Impossible d'enregistrer le type de thématique. Veuillez réessayer.");
        } finally {
            setIsLoading(false);
        }
    };
                    ...
                    <Form className="flex flex-col gap-4">
                        <div className="p-field">
                            <label htmlFor="name" className="font-medium">
                                Nom du type de thématique <span className="text-red-500">*</span>
                            </label>
                            <Field name="name">
                                {({ field }: { field: any }) => (
                                    <InputText
                            ...
                            <Field name="description">
                                {({ field }: { field: any }) => (
                                    <InputTextarea
                                        ...
                                        className={classNames('p-inputtext p-component w-full', {
                                            'border border-red-500': touched.description && errors.description,
                                        })}
                                        disabled={isLoading}
                                    />
                                )}
                        ...
                        <div className="p-field">
                            <label htmlFor="coverImage" className="font-medium">
                                Image de couverture par défaut <span className="text-red-500">*</span>
                            </label>
                            <div
                                className={`flex flex-row gap-4 items-center border ${touched.coverImage && errors.coverImage ? 'border-red-500' : 'border-gray-300'
                            ...
                            >
                                <FileUpload
                                    ...
                                    disabled={isLoading}
                                />
                                {!selectedFile ? <span>Aucun fichier sélectionné</span> : <span className='line-clamp-2 w-[70%] '>{selectedFile.name}</span>}
                            </div>
